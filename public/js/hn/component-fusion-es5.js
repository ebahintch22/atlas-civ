...
var opera_console = function () {
  ...
  return {
    ...
    addLog: function addLog(message) {
      ...
      var style = result == "success" ? "style = \"margin-top: 15px; font-weight: bold; color: '#0ce00c';\"" : result == "fail" ? "style = \"margin-top: 15px; font-weight: bold; color: red;\"" : result == "warning" ? "style = \"margin-top: 15px; font-weight: bold; color: orange;\"" : result == "request" ? "style = \"margin-top: 15px; font-weight: regular; color: cyan;\"" : "style = \"margin-top: 15px; font-weight: regular; color: white;\"";
      var tmpl = "<div ".concat(style, "><span> ").concat(_date.date(Date.now()), "  : </span><span> ").concat(message, "</span> </div>");
...
function set_time_config() {
  var dateFormat = d3.time.format("%d/%m/%Y"); //   d3.timeFormat("%d/%m/%Y");
...
var navtabController_RASS = new ui_render_navtabs("#RASS-NAV-TAB-CONTAINER", {
  ...
  tabs: [{
    ...
    html_content: " <div id=\"ADMIN-TAB-WRAPPER\" style=\"background-color: #ee8; position:relative; height: 70vh;\"> </div>",
    enabled: true
  }]
}, function (info) {
...
var navtabController_COVID_UP = new ui_render_navtabs("#COVID-NAV-TAB-UP", {
  ...
  tabs: [{
    ...
    html_content: "".concat(get_chart_container("covid-canvas-up-1", 650, 300, '95%', '25vh')),
    enabled: true
  }, {
    id: "covid-tab-up-02",
    name: "indicators_daily",
    label: "Indicateurs journaliers",
    html_content: "".concat(get_chart_container("covid-canvas-up-2", 650, 300, '95%', '25vh')),
    enabled: true
  }, {
    id: "covid-tab-up-03",
    name: "confirmed_cases_sum",
    label: "Cumul des cas confirmés",
    html_content: "".concat(get_chart_container("covid-canvas-up-3", 650, 300, '95%', '25vh')),
    enabled: true
  }]
}, function (info) {}, 1);
var navtabController_COVID_BOTTOM = new ui_render_navtabs("#COVID-NAV-TAB-BOTTOM", {
  ...
  tabs: [{
    ...
    html_content: "".concat(get_chart_container("covid-canvas-bottom-1", 650, 300, '95%', '25vh')),
    enabled: true
  }, {
    id: "covid-tab-bottom-02",
    name: "indicators_daily",
    label: "Variation quotidienne des indicateurs",
    html_content: "".concat(get_chart_container("covid-canvas-bottom-2", 650, 300, '95%', '25vh')),
    enabled: true
  }, {
    id: "covid-tab-bottom-03",
    name: "confirmed_cases_sum",
    label: "Dépistages réalisés",
    html_content: "".concat(get_chart_container("covid-canvas-bottom-3", 650, 300, '95%', '25vh')),
    enabled: true
  }]
}, function (info) {}, 1);
...
var zoom;
/*= d3.behavior.zoom().scaleExtent([1, 15])
.on('zoom', doZoom);
svg.call(zoom);*/
...
function app_start_up() {
  opera_console.addLog(JSON.stringify(user_agent));
...
function Activate_thematic_section(frame_name) {
  load_dataframe(frame_name, function (metaData) {
    ...
    update_color_palette(metaData.color_palette); //update the property window html template to the new table columnset					
...
function updateMapColors() {
  ...
  mapFeatures.selectAll('path').style('fill', function (d) {
    ...
    var attr_value = !attr_line ? 0 : getValueOfData(attr_line); //return quantize( attr_value );
...
function updateLegend(_renderer) {
  ...
  var renderer = _renderer ? _renderer : currentMetaTable.renderer;
...
function showTooltip(f) {
  ...
  var top = mouse[1] + 15;
  capture_random(left, top); // Show the tooltip (unhide it) and set the name of the data entry.
...
function user_connexion_manager_constructor() {
  ...
  function _start_session() {
    ...
    if (storeAgent.existItem("opera.kassaprekoh.2020", byPassKey)) {
      ...
      exec_infinite_safe(function () {
        Ajaxian.read("./visitors/read_visitor/".concat(user_key), restoreUserBadge, unexpected_Error_handler);
        loop_counter++;
      });
    } else {
      ...
      exec_infinite_safe(function () {
        Ajaxian.post("./visitors/new_visitor", user_agent, createUserBadge, unexpected_Error_handler);
        loop_counter++;
      });
    }
  }
  ...
  function createUserBadge(data) {
    /*=================================================================
    	create badge :  a badge is a global user containing identification 
    ...
    userBadge = data;
    app_start_up();
    start_session_notifier(userBadge);
  }

  function restoreUserBadge(data) {
    if (data.valid) {
      ...
      opera_console.addLog("Visitor with invalid KEY:" + JSON.stringify(data), "warning");
      opera_console.addLog("requesting a new KEY:" + JSON.stringify(data), "request");
      exec_infinite_safe(function () {
        Ajaxian.read("./visitors/new_visitor", createUserBadge, unexpected_Error_handler);
        loop_counter++;
      });
    }
  }
  ...
  function start_session_notifier(Badge) {
    /*=================================================================
